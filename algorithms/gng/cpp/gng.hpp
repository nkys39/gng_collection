 ...
 *
 * This implementation uses fixed-size node array with id=-1 for removed nodes,
 * matching the watanabe_gng reference implementation.
 */
...
/**
 * @brief GNG hyperparameters.
 *
 * Default values match the watanabe_gng reference implementation.
 */
struct GNGParams {
    int max_nodes = 100;    // MAX_NODE_NUM
    int lambda = 100;       // LAMBDA - node insertion interval
    float eps_b = 0.08f;    // LEARNRATE_S1 - winner learning rate
    float eps_n = 0.008f;   // LEARNRATE_S2 - neighbor learning rate
    float alpha = 0.5f;     // ALFA - error decay on split
    float beta = 0.005f;    // BETA - global error decay
    int max_age = 100;      // MAX_EDGE_AGE
};

/**
 * @brief A neuron node in the GNG network.
 *
...
public:
    ...
    using Callback = std::function<void(const GrowingNeuralGas&, int)>;
...
public:
    /**
     * @brief Construct GNG with given parameters.
     *
     * @param params GNG hyperparameters
     * @param seed Random seed (0 for random)
     */
    explicit GrowingNeuralGas(const GNGParams& params = GNGParams(), unsigned int seed = 0)
        : params(params),
...
private:
    ...
    /**
     * @brief Remove a node (only if isolated).
     */
     ...
     *
     * @return (winner_id, second_winner_id)
     */
    ...
    /**
     * @brief Single training update (follows watanabe_gng reference).
     */
    void one_train_update(const PointT& sample) {
        ...
        // Update winner error (Euclidean distance)
        Scalar dist = std::sqrt((sample - nodes[s1_id].weight).squaredNorm());
        nodes[s1_id].error += dist;
...
// Common type aliases
using GNG2f = GrowingNeuralGas<Eigen::Vector2f>;
using GNG3f = GrowingNeuralGas<Eigen::Vector3f>;
using GNG2d = GrowingNeuralGas<Eigen::Vector2d>;
using GNG3d = GrowingNeuralGas<Eigen::Vector3d>;
