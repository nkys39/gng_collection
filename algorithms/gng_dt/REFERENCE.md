...
class GNGDTParams:
    ...
    lambda_: int = 100        # ノード挿入間隔
    eps_b: float = 0.08       # 勝者学習率
    eps_n: float = 0.008      # 近傍学習率
    alpha: float = 0.5        # 分割時誤差減衰
    beta: float = 0.005       # 全体誤差減衰
    max_age: int = 100        # 最大エッジ年齢
    tau_color: float = 10.0   # 色閾値
    tau_normal: float = 0.98  # 法線閾値（内積）
```
