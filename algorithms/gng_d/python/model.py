"""Growing Neural Gas with explicit Delaunay Triangulation (GNG-D).

This variant replaces Competitive Hebbian Learning (CHL) with explicit
Delaunay triangulation for topology management using scipy.spatial.Delaunay.
...
2. Explicit Delaunay computation - used in this GNG-D variant
...
Note: This is different from GNG-T (Kubota & Satomi 2008), which uses
heuristic triangulation (quadrilateral search + intersection search)
on top of standard GNG with CHL.
"""
...
@dataclass
class NeuronNode:
    """A neuron node in the GNG-D network.
...
class GrowingNeuralGasD:
    ...
    --------
    >>> import numpy as np
    >>> from model import GrowingNeuralGasD
    >>> X = np.random.rand(1000, 2)
    >>> gng_d = GrowingNeuralGasD(n_dim=2)
    ...
    def __init__(
        ...
        Args:
            ...
            params: GNG-D hyperparameters. Uses defaults if None.
            seed: Random seed for reproducibility.
        """
        if n_dim < 2:
            raise ValueError("GNG-D requires n_dim >= 2 for Delaunay triangulation")

        self.n_dim = n_dim
        self.params = params or GNGDParams()
    ...
    def train(
        ...
        callback: Callable[[GrowingNeuralGasD, int], None] | None = None,
    ) -> GrowingNeuralGasD:
    ...
    def partial_fit(self, sample: np.ndarray) -> GrowingNeuralGasD:
...
# Aliases
GNGD = GrowingNeuralGasD
GNG_D = GrowingNeuralGasD
# Backward compatibility aliases (deprecated)
GNGTParams = GNGDParams
GrowingNeuralGasT = GrowingNeuralGasD
GNGT = GrowingNeuralGasD
GNG_T = GrowingNeuralGasD
