/**
 * @file ais_gng.hpp
 * @brief Add-if-Silent Rule-Based Growing Neural Gas (AiS-GNG) implementation
 *
 * Based on:
 *   - Shoji, M., Obo, T., & Kubota, N. (2023). "Add-if-Silent Rule-Based
 *     Growing Neural Gas for High-Density Topological Structure"
 *     IEEE RO-MAN 2023.
 *
 * AiS-GNG extends GNG-U2 with the Add-if-Silent rule, which directly adds
 * input data as new nodes when certain conditions are met.
 */
...
struct AiSGNGParams {
    ...
    int kappa = 10;
    // Add-if-Silent rule tolerances
    float theta_ais_min = 0.03f;  // Minimum distance threshold
    float theta_ais_max = 0.15f;  // Maximum distance threshold
};

/**
 * @brief A neuron node in the AiS-GNG network.
 */
template <typename PointT>
struct NeuronNodeAiS {
    int id = -1;
    float error = 1.0f;
    float utility = 0.0f;
    PointT weight;

    NeuronNodeAiS() = default;
    NeuronNodeAiS(int id_, const PointT& weight_, float error_ = 1.0f, float utility_ = 0.0f)
        : id(id_), error(error_), utility(utility_), weight(weight_) {}
};

/**
 * @brief AiS-GNG algorithm implementation.
 *
 * @tparam PointT Point type (Eigen vector)
 */
template <typename PointT>
class AddIfSilentGNG {
public:
    using Scalar = typename PointT::Scalar;
    using Callback = std::function<void(const AddIfSilentGNG&, int)>;

    AiSGNGParams params;
    std::vector<NeuronNodeAiS<PointT>> nodes;
    std::unordered_map<int, std::unordered_set<int>> edges_per_node;
    Eigen::MatrixXi edges;
    int n_learning = 0;
    int n_ais_additions = 0;     // Add-if-Silent additions
    int n_utility_removals = 0;  // Utility-based removals
...
public:
    explicit AddIfSilentGNG(const AiSGNGParams& params = AiSGNGParams(), unsigned int seed = 0)
        : params(params),
...
private:
    int add_node(const PointT& weight, float error = 1.0f, float utility = 0.0f) {
        ...
        nodes[node_id] = NeuronNodeAiS<PointT>(node_id, weight, error, utility);
     ...
     *
     * If the winner distance is in range [theta_min, theta_max],
     * add input directly as a new node.
     *
     * @return true if a node was added via AiS rule
     */
    bool ais_growing_process(const PointT& sample, int s1_id, int s2_id,
        ...
        // Add-if-Silent condition: theta_min <= dist1 <= theta_max
        if (dist1 >= params.theta_ais_min && dist1 <= params.theta_ais_max) {
    ...
    void one_train_update(const PointT& sample) {
        ...
        // Periodically add new node (skip if AiS already added one)
        n_trial_++;
...
// Common type aliases
using AiSGNG2f = AddIfSilentGNG<Eigen::Vector2f>;
using AiSGNG3f = AddIfSilentGNG<Eigen::Vector3f>;
using AiSGNG2d = AddIfSilentGNG<Eigen::Vector2d>;
using AiSGNG3d = AddIfSilentGNG<Eigen::Vector3d>;
