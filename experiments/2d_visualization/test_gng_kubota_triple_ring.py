"""Test GNG Kubota (paper-compliant) algorithm on triple ring data with GIF visualization.

Usage:
    python test_gng_kubota_triple_ring.py
"""
...
from model_kubota import GNGKubota, GNGKubotaParams
...
def create_frame(
    ...
    ax.set_aspect("equal")
    ax.set_title(f"GNG Kubota - Iteration {iteration} ({len(nodes)} nodes)")
...
def run_experiment(
    ...
    gif_frames: int = 100,
    output_gif: str = "gng_kubota_triple_ring_growth.gif",
    output_final: str = "gng_kubota_triple_ring_final.png",
    seed: int = 42,
) -> None:
    """Run GNG Kubota experiment with visualization."""
    np.random.seed(seed)
    ...
    # Setup GNG Kubota with parameters
    params = GNGKubotaParams(
        max_nodes=100,
    ...
    gng = GNGKubota(n_dim=2, params=params, seed=seed)
    ...
    # Train
    print(f"Training GNG Kubota for {n_iterations} iterations...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test GNG Kubota on triple ring data")
    ...
    parser.add_argument(
        "--output-final", type=str, default="triple_ring_final.png", help="Output final image path"
    )
