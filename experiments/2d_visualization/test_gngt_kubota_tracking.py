"""Test GNG-T Kubota dynamic tracking on moving ring."""
...
def create_tracking_frame(ax, ring_center, ring_r_inner, ring_r_outer,
                          current_samples, nodes, edges, triangles,
                          orbit_center, orbit_radius, frame, total_frames):
    ax.clear()
    ...
    if triangles:
        for tri in triangles:
            triangle = plt.Polygon(nodes[list(tri)], fill=True, facecolor="lightgreen",
                                   edgecolor="green", alpha=0.2, linewidth=0.5)
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.set_aspect("equal")
    ax.set_title(f"GNG-T Kubota Tracking - Frame {frame}/{total_frames}")


def run_tracking_experiment(output_gif="gngt_kubota_tracking.gif", seed=42):
    ...
    params = GNGTKubotaParams(
        ...
        alpha=0.5, beta=0.01, max_age=30,
    )
    ...
    print(f"Running GNG-T Kubota tracking ({total_frames} frames)...")
    ...
    for frame in range(total_frames):
        ...
        triangles = gng_t.get_triangles()
        create_tracking_frame(ax, ring_center, ring_r_inner, ring_r_outer, samples,
                              nodes, edges, triangles, orbit_center, orbit_radius,
