"""Test GCS dynamic tracking on moving ring."""
...
def create_tracking_frame(
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.set_aspect("equal")
    ax.set_title(f"GCS Tracking - Frame {frame}/{total_frames}")
...
def run_tracking_experiment(
    ...
    output_gif: str = "gcs_tracking.gif",
    seed: int = 42,
) -> None:
    ...
    print(f"Running GCS tracking experiment ({total_frames} frames)...")
