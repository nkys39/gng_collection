"""Test GCS Kubota (paper-compliant) on triple ring data with GIF visualization."""
...
def create_frame(
    ...
    ax.set_ylim(1, 0)
    ax.set_aspect("equal")
    ax.set_title(f"GCS Kubota - Iteration {iteration} ({len(nodes)} nodes, {len(edges)} edges)")
...
def run_experiment(
    ...
    gif_frames: int = 100,
    output_gif: str = "gcs_kubota_triple_ring_growth.gif",
    output_final: str = "gcs_kubota_triple_ring_final.png",
    seed: int = 42,
) -> None:
    """Run GCS Kubota experiment."""
    np.random.seed(seed)
    ...
    print(f"Training GCS Kubota for {n_iterations} iterations...")
