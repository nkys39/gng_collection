"""Test GNG-U dynamic tracking on moving ring.

The ring (input data distribution) moves along a circular orbit,
and GNG-U learns online to track the moving distribution.

GNG-U is expected to perform better than standard GNG for tracking
because it can remove nodes that become obsolete as the distribution moves.
"""
...
def create_tracking_frame(
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.set_aspect("equal")
    ax.set_title(f"GNG-U Tracking - Frame {frame}/{total_frames} ({n_removals} removed)")
...
def run_tracking_experiment(
    ...
    output_gif: str = "gngu_tracking.gif",
    seed: int = 42,
    utility_k: float = 1.3,
) -> None:
    """Run GNG-U tracking experiment.
    ...
    # Setup GNG-U with parameters tuned for tracking
    params = GNGUParams(
        max_nodes=50,
        ...
        utility_k=utility_k,  # GNG-U: utility threshold
    )
    ...
    print(f"Running GNG-U tracking experiment ({total_frames} frames)...")
    print(f"Ring moves along orbit: center={orbit_center}, radius={orbit_radius}")
    print(f"GNG-U params: lambda={params.lambda_}, eps_b={params.eps_b}, max_age={params.max_age}, utility_k={params.utility_k}")
    ...
    for frame in range(total_frames):
        ...
        # Train GNG-U on current samples
        for sample in samples:
            gng.partial_fit(sample)

        # Get current GNG-U state
        ...
        if (frame + 1) % 20 == 0:
            print(f"  Frame {frame + 1}/{total_frames}: {len(nodes)} nodes, {len(edges)} edges, {gng.n_removals} removed")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test GNG-U tracking on moving ring")
    ...
    parser.add_argument("--output", type=str, default="gngu_tracking.gif", help="Output GIF")
