"""Test Neural Gas dynamic tracking on moving ring."""
...
def create_tracking_frame(
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.set_aspect("equal")
    ax.set_title(f"Neural Gas Tracking - Frame {frame}/{total_frames}")
...
def run_tracking_experiment(
    ...
    output_gif: str = "ng_tracking.gif",
    seed: int = 42,
    n_nodes: int = 30,
) -> None:
    ...
    print(f"Running Neural Gas tracking experiment ({total_frames} frames)...")
    ...
    for frame in range(total_frames):
        ...
        if (frame + 1) % 20 == 0:
            print(f"  Frame {frame + 1}/{total_frames}: {len(edges)} edges")
