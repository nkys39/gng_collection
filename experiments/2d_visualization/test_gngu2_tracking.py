"""Test GNG-U2 dynamic tracking on moving ring.

The ring (input data distribution) moves along a circular orbit,
and GNG-U2 learns online to track the moving distribution.

GNG-U2's κ-interval utility checks enable faster adaptation to
non-stationary distributions compared to standard GNG-U.
"""
...
def create_tracking_frame(
    ...
    ax.scatter(
        ...
        s=40,
        zorder=5,
        label=f"GNG-U2 ({len(nodes)} nodes)",
    )
    ...
    ax.set_aspect("equal")
    ax.set_title(
        f"GNG-U2 Tracking - Frame {frame}/{total_frames} "
...
def run_tracking_experiment(
    ...
    output_gif: str = "gngu2_tracking.gif",
    seed: int = 42,
) -> None:
    """Run GNG-U2 tracking experiment.
    ...
    # Setup GNG-U2 with parameters tuned for tracking
    params = GNGU2Params(
        max_nodes=50,
        ...
        kappa=5,          # More frequent utility checks (GNG-U2 feature)
    )
    ...
    print(f"Running GNG-U2 tracking experiment ({total_frames} frames)...")
    print(f"Ring moves along orbit: center={orbit_center}, radius={orbit_radius}")
    print(f"GNG-U2 params: lambda={params.lambda_}, kappa={params.kappa}, eps_b={params.eps_b}, max_age={params.max_age}")
    ...
    for frame in range(total_frames):
        ...
        # Train GNG-U2 on current samples
        for sample in samples:
            gng.partial_fit(sample)

        # Get current GNG-U2 state
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test GNG-U2 tracking on moving ring")
    ...
    parser.add_argument("--output", type=str, default="gngu2_tracking.gif", help="Output GIF")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--kappa", type=int, default=5, help="Utility check interval")
    parser.add_argument("--utility-k", type=float, default=500.0, help="Utility threshold")
