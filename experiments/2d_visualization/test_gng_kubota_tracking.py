"""Test GNG Kubota dynamic tracking on moving ring."""
...
def create_tracking_frame(ax, ring_center, ring_r_inner, ring_r_outer,
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.set_aspect("equal")
    ax.set_title(f"GNG Kubota Tracking - Frame {frame}/{total_frames}")


def run_tracking_experiment(output_gif="gng_kubota_tracking.gif", seed=42):
    ...
    print(f"Running GNG Kubota tracking ({total_frames} frames)...")
