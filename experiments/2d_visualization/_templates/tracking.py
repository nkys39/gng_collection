"""Test [ALGORITHM_NAME] dynamic tracking on moving ring.

Template for tracking non-stationary distribution test.
The ring (input data distribution) moves along a circular orbit,
and the algorithm learns online to track the moving distribution.

Usage:
    1. Copy this file to test_[algorithm]_tracking.py
    2. Update imports and class names
    3. Adjust parameters if needed
    4. Run: python test_[algorithm]_tracking.py

Output:
    - [algorithm]_tracking.gif: Tracking animation
"""
...
def create_tracking_frame(
    ...
    Args:
        ax: Matplotlib axes.
        ring_center: Current ring center position.
        ring_r_inner: Ring inner radius.
        ring_r_outer: Ring outer radius.
        current_samples: Current batch of samples.
        nodes: Node positions array.
        edges: List of edge tuples.
        orbit_center: Center of the orbit path.
    ...
    title = f"[ALGORITHM] Tracking - Frame {frame}/{total_frames}"
    if extra_info:
        title += f" [{extra_info}]"
    ax.set_title(title)
...
def run_tracking_experiment(
    ...
    output_gif: str = "[algorithm]_tracking.gif",
    seed: int = 42,
) -> None:
    ...
    # UPDATE: Setup algorithm with tracking-optimized parameters
    ...
    print(f"Running [ALGORITHM] tracking experiment ({total_frames} frames)...")
    ...
    for frame in range(total_frames):
        ...
        # Train model on current samples
        ...
        extra_info = ""  # e.g., f"{model.n_removals} removed"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test [ALGORITHM] tracking on moving ring")
    ...
    parser.add_argument("--output", type=str, default="[algorithm]_tracking.gif", help="Output GIF")
