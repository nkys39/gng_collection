/**
 * @file test_ng_triple_ring.cpp
 * @brief Test Neural Gas on triple ring data (C++ implementation)
 *
 * Generates triple ring sample data, runs Neural Gas training,
...
void save_graph(const neural_gas::NG2f& ng, const std::string& nodes_path, const std::string& edges_path) {
    std::vector<Eigen::Vector2f> nodes;
    std::vector<std::pair<int, int>> edges;
    ng.get_graph(nodes, edges);
...
int main(int argc, char* argv[]) {
    // Default parameters
    std::string output_dir = "ng_triple_ring_output";
    int n_samples = 1500;
    int n_iterations = 5000;
    unsigned int seed = 42;
    int frame_interval = 50;  // Save every 50 iterations for ~100 frames
    ...
    std::cout << "Neural Gas Triple Ring Test (C++)\n";
    std::cout << "=================================\n";
    ...
    // Setup Neural Gas (parameters matching Python version)
    neural_gas::NGParams params;
    params.n_nodes = 50;
    params.lambda_initial = 10.0f;
    params.lambda_final = 0.1f;
    params.eps_initial = 0.5f;
    params.eps_final = 0.005f;
    params.max_age = 50;
    params.use_chl = true;

    std::cout << "Neural Gas Parameters:\n";
    std::cout << "  n_nodes: " << params.n_nodes << "\n";
    std::cout << "  lambda_initial: " << params.lambda_initial << "\n";
    std::cout << "  lambda_final: " << params.lambda_final << "\n";
    std::cout << "  eps_initial: " << params.eps_initial << "\n";
    std::cout << "  eps_final: " << params.eps_final << "\n";
    std::cout << "  max_age: " << params.max_age << "\n";
    std::cout << "  use_chl: " << (params.use_chl ? "true" : "false") << "\n\n";

    neural_gas::NG2f model(params, seed);
    ...
    auto callback = [&](const neural_gas::NG2f& ng, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            std::stringstream ss;
            ss << std::setfill('0') << std::setw(5) << iter;
            std::string suffix = ss.str();

            save_graph(ng, output_dir + "/nodes_" + suffix + ".csv",
