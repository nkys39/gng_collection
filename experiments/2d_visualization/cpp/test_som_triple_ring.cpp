/**
 * @file test_som_triple_ring.cpp
 * @brief Test SOM on triple ring data (C++ implementation)
 *
 * Generates triple ring sample data, runs SOM training,
...
void save_graph(const som::SOM2f& model, const std::string& nodes_path, const std::string& edges_path) {
...
int main(int argc, char* argv[]) {
    // Default parameters
    std::string output_dir = "som_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "SOM Triple Ring Test (C++)\n";
    std::cout << "==========================\n";
    ...
    // Setup SOM (parameters matching Python version)
    som::SOMParams params;
    params.grid_height = 10;
    params.grid_width = 10;
    params.sigma_initial = 5.0f;
    params.sigma_final = 0.5f;
    params.learning_rate_initial = 0.5f;
    params.learning_rate_final = 0.01f;

    std::cout << "SOM Parameters:\n";
    std::cout << "  grid_height: " << params.grid_height << "\n";
    std::cout << "  grid_width: " << params.grid_width << "\n";
    std::cout << "  sigma_initial: " << params.sigma_initial << "\n";
    std::cout << "  sigma_final: " << params.sigma_final << "\n";
    std::cout << "  learning_rate_initial: " << params.learning_rate_initial << "\n";
    std::cout << "  learning_rate_final: " << params.learning_rate_final << "\n\n";

    som::SOM2f model(params, seed);
    ...
    auto callback = [&](const som::SOM2f& som, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            std::stringstream ss;
            ss << std::setfill('0') << std::setw(5) << iter;
            std::string suffix = ss.str();

            save_graph(som, output_dir + "/nodes_" + suffix + ".csv",
