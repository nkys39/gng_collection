/**
 * @file test_hcl_triple_ring.cpp
 * @brief Test HCL on triple ring data (C++ implementation)
 *
 * Generates triple ring sample data, runs HCL training,
...
void save_graph(const hcl::HCL2f& model, const std::string& nodes_path, const std::string& edges_path) {
...
int main(int argc, char* argv[]) {
    // Default parameters
    std::string output_dir = "hcl_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "HCL Triple Ring Test (C++)\n";
    std::cout << "==========================\n";
    ...
    // Setup HCL (parameters matching Python version)
    hcl::HCLParams params;
    params.n_nodes = 50;
    params.learning_rate_initial = 0.5f;
    params.learning_rate_final = 0.01f;

    std::cout << "HCL Parameters:\n";
    ...
    auto callback = [&](const hcl::HCL2f& hcl, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            std::stringstream ss;
            ss << std::setfill('0') << std::setw(5) << iter;
            std::string suffix = ss.str();

            save_graph(hcl, output_dir + "/nodes_" + suffix + ".csv",
