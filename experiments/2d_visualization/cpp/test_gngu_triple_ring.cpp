/**
 * @file test_gngu_triple_ring.cpp
 * @brief Test GNG-U on triple ring data (C++ implementation)
 *
 * Generates triple ring sample data, runs GNG-U training,
...
void save_graph(const gng_u::GNGU2f& gngu, const std::string& nodes_path, const std::string& edges_path) {
    std::vector<Eigen::Vector2f> nodes;
    std::vector<std::pair<int, int>> edges;
    gngu.get_graph(nodes, edges);
...
int main(int argc, char* argv[]) {
    // Default parameters
    std::string output_dir = "gngu_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "GNG-U Triple Ring Test (C++)\n";
    std::cout << "============================\n";
    ...
    // Setup GNG-U (parameters matching Python version)
    ...
    params.beta = 0.005f;
    params.max_age = 100;
    params.utility_k = 1.3f;  // GNG-U specific
    ...
    gng_u::GNGU2f model(params, seed);
    model.init();
    ...
    auto callback = [&](const gng_u::GNGU2f& gngu, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            std::stringstream ss;
            ss << std::setfill('0') << std::setw(5) << iter;
            std::string suffix = ss.str();

            save_graph(gngu, output_dir + "/nodes_" + suffix + ".csv",
                       output_dir + "/edges_" + suffix + ".csv");

            frame_iterations.push_back(iter);

            std::cout << "Iteration " << iter << ": "
                      << gngu.num_nodes() << " nodes, "
                      << gngu.num_edges() << " edges, "
                      << gngu.n_removals << " removals\n";
        }
    };
    ...
    // Save metadata
    {
        ...
        file << "n_removals," << model.n_removals << "\n";
    }

    std::cout << "\nTraining complete!\n";
    std::cout << "Time: " << duration.count() << " ms\n";
    std::cout << "Final: " << model.num_nodes() << " nodes, " << model.num_edges() << " edges\n";
    std::cout << "Utility-based removals: " << model.n_removals << "\n";
