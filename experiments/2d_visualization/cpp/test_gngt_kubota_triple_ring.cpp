/**
 * @file test_gngt_kubota_triple_ring.cpp
 * @brief Test GNG-T Kubota (paper-compliant) on triple ring data
 *
 * Uses longest edge selection for node insertion and γ formula for intersection detection.
 */
...
void save_graph(const gng_t_kubota::GNGTKubota<Eigen::Vector2f>& gng,
...
int main(int argc, char* argv[]) {
    std::string output_dir = "gngt_kubota_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "GNG-T Kubota Triple Ring Test (C++)\n";
    std::cout << "====================================\n";
    ...
    gng_t_kubota::GNGTKubotaParams params;
    ...
    std::cout << "GNG-T Kubota Parameters:\n";
    ...
    gng_t_kubota::GNGTKubota<Eigen::Vector2f> model(params, seed);
    model.init();
    ...
    auto callback = [&](const gng_t_kubota::GNGTKubota<Eigen::Vector2f>& gng, int iter) {
