/**
 * @file test_gcs_triple_ring.cpp
 * @brief Test GCS on triple ring data (C++ implementation)
 *
 * Generates triple ring sample data, runs GCS training,
...
void save_graph(const gcs::GCS2f& model, const std::string& nodes_path, const std::string& edges_path) {
...
int main(int argc, char* argv[]) {
    // Default parameters
    std::string output_dir = "gcs_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "GCS Triple Ring Test (C++)\n";
    std::cout << "==========================\n";
    ...
    // Setup GCS (parameters matching Python version)
    gcs::GCSParams params;
    ...
    auto callback = [&](const gcs::GCS2f& gcs, int iter) {
