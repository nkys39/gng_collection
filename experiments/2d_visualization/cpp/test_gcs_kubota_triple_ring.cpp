/**
 * @file test_gcs_kubota_triple_ring.cpp
 * @brief Test GCS Kubota (paper-compliant) on triple ring data
 *
 * Uses longest edge selection for node insertion instead of max error neighbor.
 */
...
void save_graph(const gcs_kubota::GCSKubota<Eigen::Vector2f>& model,
...
int main(int argc, char* argv[]) {
    std::string output_dir = "gcs_kubota_triple_ring_output";
    int n_samples = 1500;
    ...
    std::cout << "GCS Kubota Triple Ring Test (C++)\n";
    std::cout << "==================================\n";
    ...
    gcs_kubota::GCSKubotaParams params;
    ...
    std::cout << "GCS Kubota Parameters:\n";
    ...
    gcs_kubota::GCSKubota<Eigen::Vector2f> model(params, seed);
    ...
    auto callback = [&](const gcs_kubota::GCSKubota<Eigen::Vector2f>& gcs, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            std::stringstream ss;
            ss << std::setfill('0') << std::setw(5) << iter;
            std::string suffix = ss.str();

            save_graph(gcs, output_dir + "/nodes_" + suffix + ".csv",
