"""Test GNG-T Kubota (paper-compliant) learning on triple ring pattern.

GNG-T Kubota uses the exact algorithms from Kubota & Satomi (2008):
- Longest edge selection for node insertion (instead of max error neighbor)
- γ formula for intersection detection (instead of CCW method)
"""
...
from model_kubota import GNGTKubota, GNGTKubotaParams
...
def create_visualization_frame(
    ...
    ax.set_ylim(1, 0)
    ax.set_aspect("equal")
    ax.set_title(f"GNG-T Kubota - Iter {iteration} ({len(nodes)} nodes, {len(edges)} edges)")
...
def run_experiment(
    ...
    n_frames: int = 50,
    n_samples: int = 1500,
    output_gif: str = "gngt_kubota_triple_ring_growth.gif",
    output_png: str = "gngt_kubota_triple_ring_final.png",
    seed: int = 42,
    show_triangles: bool = True,
) -> None:
    """Run GNG-T Kubota triple ring experiment."""
    ...
    gng_t = GNGTKubota(n_dim=2, params=params, seed=seed)

    print(f"Training GNG-T Kubota for {n_iterations} iterations...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test GNG-T Kubota on triple ring")
