...
Furuta手法の実装は論文のアルゴリズムに基づいていますが、スパースなGNGグラフでの安定性を確保するために以下の拡張を追加しています。

#### 論文通りの実装部分 ✓

| 項目 | 実装 | 論文 |
|------|------|------|
| 方向更新式 | `V_new = (best_angle - 225) % 360` | V_new = (V_old - 225) mod 360 |
| CCW最近傍 | `(angle - direction) % 360` 最小 | CCW方向で最も近い隣接ノード |
| 外側輪郭開始 | Y_min ノード、方向180° | Y座標最小ノード、探索方向180° |
| 内側輪郭閾値 | 135° | θ_thv = 135° |

#### 実装で追加した拡張（論文にない）⚠️

論文では安定性の問題を指摘していますが、具体的な対策は示されていません。
本実装では以下の対策を追加しています：

| 拡張 | 理由 | コード位置 |
|------|------|-----------|
| **開始ノードの近傍数チェック** | 近傍1つではCCW追跡が不可能 | `_detect_outer_contour` L161-166 |
| **前ノード除外** | 即座に元のノードに戻るのを防止 | `_find_next_ccw` L257-260 |
| **最小輪郭長チェック** | 2ノードで早期終了するのを防止 | `_detect_outer_contour` L190, 209 |
| **サブループ検出** | 開始ノード以外への再訪問で強制終了 | `_detect_outer_contour` L214-218 |

これらの拡張により、論文で報告されている計算時間比（1.86倍）とほぼ同等の結果（1.89〜2.00倍）を再現できています。
