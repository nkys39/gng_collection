"""Test DD-GNG with full features: manual attention + auto-detection.

Demonstrates all DD-GNG capabilities:
- Manual attention region at floor-wall edge
- Automatic attention detection via surface classification
- Strength-weighted learning and insertion
- Surface type visualization (plane/edge/corner)

Usage:
    python test_ddgng_full_features.py
"""
...
def create_frame(
    ...
    ax.set_title(
        f"DD-GNG Full Features - Iter {iteration}\n"
...
def run_experiment(
    ...
    gif_frames: int = 80,
    output_gif: str = "ddgng_full_features_growth.gif",
    output_final: str = "ddgng_full_features_final.png",
    seed: int = 42,
) -> None:
    """Run DD-GNG with full features.
    ...
    # DD-GNG with FULL FEATURES: manual + auto-detection
    params = DDGNGParams(
        max_nodes=150,
        lambda_=100,
        eps_b=0.1,
    ...
    # Add MANUAL attention region at the edge
    ...
    print("  - Manual attention region at floor-wall edge")
    print("  - Auto-detection of stable corners")
    ...
    def callback(m, iteration):
        if iteration % frame_interval == 0 or iteration == n_iterations - 1:
            ...
            create_frame(
                ...
                m.attention_regions, m.n_auto_attention,
                azim=azim,
            )
            fig.canvas.draw()
    ...
    for region in model.attention_regions:
        draw_attention_region_3d(ax1, region.center, region.size)
    ...
    ax1.set_title(
        f"DD-GNG Full Features - 3D View\n"
        f"{len(nodes)} nodes, {model.n_auto_attention} auto-detected\n"
        f"Green=Plane, Yellow=Edge, Red=Corner, △=Auto-attention"
    )
    ...
    # Draw manual attention region in 2D
    for region in model.attention_regions:
        rect = plt.Rectangle(
            (region.center[1] - region.size[1], region.center[2] - region.size[2]),
            region.size[1] * 2, region.size[2] * 2,
    ...
    ax2.set_title(
        f"DD-GNG Full Features - Side View (YZ)\n"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test DD-GNG with full features (manual + auto-detection)"
    )
    parser.add_argument("-n", "--n-samples", type=int, default=2000)
    parser.add_argument("--iterations", type=int, default=8000)
    parser.add_argument("--frames", type=int, default=80)
    parser.add_argument("--output-gif", type=str, default="ddgng_full_features_growth.gif")
    parser.add_argument("--output-final", type=str, default="ddgng_full_features_final.png")
