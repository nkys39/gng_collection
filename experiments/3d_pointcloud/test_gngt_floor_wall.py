"""Test GNG-T algorithm on 3D floor and wall data with GIF visualization.

Usage:
    python test_gngt_floor_wall.py
"""
...
def create_frame(ax, points, nodes, edges, iteration, elev=25, azim=120):
    ...
    ax.set_title(f"GNG-T 3D Floor+Wall - Iter {iteration} ({len(nodes)} nodes)")
    ax.legend(loc="upper right")
    ax.view_init(elev=elev, azim=azim)


def run_experiment(n_samples=2000, n_iterations=8000, gif_frames=100,
                   output_gif="gngt_floor_wall_growth.gif",
                   output_final="gngt_floor_wall_final.png", seed=42):
    np.random.seed(seed)
    ...
    params = GNGTParams(
        ...
        alpha=0.5, beta=0.005, max_age=100,
    )
    gng = GrowingNeuralGasT(n_dim=3, params=params, seed=seed)
    ...
    ax_3d.set_title(f"GNG-T 3D - Perspective View ({len(nodes)} nodes)")
    ...
    ax_2d.set_title(f"GNG-T 3D - Side View (YZ plane)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-gif", type=str, default="gngt_floor_wall_growth.gif")
    parser.add_argument("--output-final", type=str, default="gngt_floor_wall_final.png")
