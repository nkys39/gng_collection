"""Test AiS-GNG-DT algorithm on 3D floor and wall data.
...
This test compares the learning behavior with the standard GNG-DT,
expecting faster structure generation due to the Add-if-Silent rule.

Usage:
    python test_aisgngdt_floor_wall.py
"""
...
sys.path.insert(0, str(Path(__file__).parents[2] / "algorithms" / "ais_gng_dt" / "python"))
sys.path.insert(0, str(Path(__file__).parents[2] / "data" / "3d"))

from model import AiSGNGDT, AiSGNGDTParams
...
def create_frame(
    ...
    ax.set_title(
        f"AiS-GNG-DT Floor+Wall - Iter {iteration}\n"
        f"({len(nodes)} nodes, {len(pos_edges)} pos-edges, AiS: {n_ais_additions})"
    )
...
def run_experiment(
    ...
    gif_frames: int = 100,
    output_gif: str = "aisgngdt_floor_wall_growth.gif",
    output_final: str = "aisgngdt_floor_wall_final.png",
    seed: int = 42,
) -> None:
    """Run AiS-GNG-DT 3D experiment.
    ...
    params = AiSGNGDTParams(
        ...
        tau_normal=0.95,
        # AiS-GNG specific
        theta_ais_min=0.03,  # Minimum distance for AiS rule
        theta_ais_max=0.12,  # Maximum distance for AiS rule
        kappa=10,            # Utility check interval
        utility_k=1000.0,    # Utility removal threshold
        chi=0.005,           # Utility decay rate
    )
    gng = AiSGNGDT(params=params, seed=seed)
    ...
    def callback(model, iteration):
        if iteration % frame_interval == 0 or iteration == n_iterations - 1:
            ...
            create_frame(
                ...
                normals,
                iteration,
                n_ais_additions=model.n_ais_additions,
                show_normals=False,
                azim=azim,
            )
            fig.canvas.draw()
            ...
            print(
                ...
                f"AiS additions: {model.n_ais_additions}"
            )
            frame_count[0] += 1

    # Train
    print(f"Training AiS-GNG-DT for {n_iterations} iterations...")
    print(
        f"Parameters: max_nodes={params.max_nodes}, lambda={params.lambda_}, "
        f"tau_normal={params.tau_normal}, theta_ais=[{params.theta_ais_min}, {params.theta_ais_max}]"
    )
    gng.train(points, n_iterations=n_iterations, callback=callback)

    # Final statistics
    print(f"\nFinal Statistics:")
    print(f"  Total AiS additions: {gng.n_ais_additions}")
    print(f"  Utility removals: {gng.n_utility_removals}")
    ...
    # Right: 2D side view (YZ plane) with statistics
    ...
    ax_2d.set_title(
        f"Side View (YZ)\n"
        f"AiS additions: {gng.n_ais_additions}, Utility removals: {gng.n_utility_removals}"
    )
    ax_2d.legend(loc="upper left")

    plt.suptitle(
        f"AiS-GNG-DT: {len(nodes)} nodes | "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test AiS-GNG-DT on 3D floor and wall data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-gif",
        type=str,
        default="aisgngdt_floor_wall_growth.gif",
        help="Output GIF path",
    )
    parser.add_argument(
        "--output-final",
        type=str,
        default="aisgngdt_floor_wall_final.png",
