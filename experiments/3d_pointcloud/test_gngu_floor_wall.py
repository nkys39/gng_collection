"""Test GNG-U algorithm on 3D floor and wall data with GIF visualization.

Creates a visualization of GNG-U learning on a floor-wall L-shaped surface.

Usage:
    python test_gngu_floor_wall.py
"""
...
def create_frame(
    ...
    ax.set_title(f"GNG-U 3D Floor+Wall - Iter {iteration} ({len(nodes)} nodes)")
...
def run_experiment(
    ...
    gif_frames: int = 100,
    output_gif: str = "gngu_floor_wall_growth.gif",
    output_final: str = "gngu_floor_wall_final.png",
    seed: int = 42,
) -> None:
    """Run GNG-U 3D experiment with visualization."""
    np.random.seed(seed)
    ...
    # Setup GNG-U with parameters tuned for 3D
    params = GNGUParams(
        max_nodes=150,
        lambda_=100,
        eps_b=0.1,
    ...
    gng = GrowingNeuralGasU(n_dim=3, params=params, seed=seed)
    ...
    ax_3d.set_title(f"GNG-U 3D - Perspective View ({len(nodes)} nodes)")
    ...
    ax_2d.set_title(f"GNG-U 3D - Side View (YZ plane)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Test GNG-U on 3D floor and wall data")
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-gif",
        type=str,
        default="gngu_floor_wall_growth.gif",
        help="Output GIF path",
    )
    parser.add_argument(
        "--output-final",
        type=str,
        default="gngu_floor_wall_final.png",
