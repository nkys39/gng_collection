"""Test AiS-GNG algorithm on 3D floor and wall data with GIF visualization.

Usage:
    python test_aisgng_floor_wall.py
"""
...
def create_frame(ax, points, nodes, edges, iteration, elev=25, azim=120):
    ...
    ax.set_title(f"AiS-GNG 3D Floor+Wall - Iter {iteration} ({len(nodes)} nodes)")
    ax.legend(loc="upper right")
    ax.view_init(elev=elev, azim=azim)


def run_experiment(n_samples=2000, n_iterations=8000, gif_frames=100,
                   output_gif="aisgng_floor_wall_growth.gif",
                   output_final="aisgng_floor_wall_final.png", seed=42):
    np.random.seed(seed)
    ...
    params = AiSGNGParams(
        ...
        alpha=0.5, beta=0.005, chi=0.005, max_age=100,
        utility_k=1000.0, kappa=10,
        theta_ais_min=0.05, theta_ais_max=0.20,  # Scaled for 3D
    )
    gng = AiSGNG(n_dim=3, params=params, seed=seed)
    ...
    ax_3d.set_title(f"AiS-GNG 3D - Perspective View ({len(nodes)} nodes)")
    ...
    ax_2d.set_title(f"AiS-GNG 3D - Side View (YZ plane)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-gif", type=str, default="aisgng_floor_wall_growth.gif")
    parser.add_argument("--output-final", type=str, default="aisgng_floor_wall_final.png")
