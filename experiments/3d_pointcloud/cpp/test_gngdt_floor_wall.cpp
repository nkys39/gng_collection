/**
 * @file test_gngdt_floor_wall.cpp
 * @brief Test GNG-DT on 3D floor and wall data (C++ implementation)
 *
 * Tests GNG-DT with multiple edge topologies (position, color, normal).
 */
...
int main(int argc, char* argv[]) {
    std::string output_dir = "gngdt_floor_wall_output";
    int n_samples = 2000;
    ...
    std::cout << "GNG-DT 3D Floor+Wall Test (C++)\n";
    std::cout << "===============================\n";
    ...
    // GNG-DT parameters (matching original toda_gngdt)
    gng_dt::GNGDTParams params;
    ...
    gng_dt::GrowingNeuralGasDT model(params, seed);
    model.init();
    ...
    auto callback = [&](const gng_dt::GrowingNeuralGasDT& gng, int iter) {
        if (iter % frame_interval == 0 || iter == n_iterations - 1) {
            csv_writer::save_gngdt_graph_3d(gng, output_dir, iter);
