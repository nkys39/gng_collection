...
@dataclass
class GNGU2SquaredParams:
    """GNG-U2 (Squared) hyperparameters."""
    ...
    chi: float = 0.005
    max_age: int = 88
    utility_k: float = 1.3  # Adjusted for squared distance scale (like GNG-U)
    kappa: int = 10
...
class GNGU2Squared:
    """GNG-U2 with squared distance metric.

    Uses squared distance for error/utility calculations to match GNG-U.
    Keeps kappa-interval utility checks from GNG-U2.
    """

    def __init__(
        self,
        n_dim: int = 2,
        params: GNGU2SquaredParams | None = None,
        seed: int | None = None,
    ):
        self.n_dim = n_dim
        self.params = params or GNGU2SquaredParams()
    ...
    def _find_two_nearest(self, x: np.ndarray) -> tuple[int, int, float, float]:
        ...
        # Return SQUARED distances (no sqrt)
    ...
    def _one_train_update(self, sample: np.ndarray) -> None:
        ...
        # kappa-interval utility check (GNG-U2 feature)
    ...
    def train(
        ...
        callback: Callable[[GNGU2Squared, int], None] | None = None,
    ) -> GNGU2Squared:
        n_samples = len(data)
    ...
    def partial_fit(self, sample: np.ndarray) -> GNGU2Squared:
