...
### 8.5 Lazy Errorの制限事項（重要）

**発見**: 論文のLazy Error公式は、標準GNGのエラー処理と**完全に等価ではない**。

#### 数学的分析

**標準GNG**（勝者になったステップsで、サイクル内）:
```
手順: 毎ステップ全ノード減衰 → 勝者探索 → 勝者にエラー加算

サイクル終了時のエラー値:
E_final = E_init × β^λ + dist_sq × β^(λ-s-1)

ここで:
- β = 1 - β_std（標準GNGのβ_std=0.0005なら、β=0.9995）
- λ = サイクル長（例: 100）
- s = 勝利したステップ（0からλ-1）
```

**論文のLazy Error (Algorithm 4)**:
```
inc_error*(c, s, ν, v):
  E_ν = β^(λ-s) × E_ν + v

サイクル終了時のエラー値:
E_final = E_init × β^(λ-s) + dist_sq
```

#### 差異の原因

| 項目 | 標準GNG | 論文Lazy Error | 差異 |
|------|---------|---------------|------|
| 既存エラーへの減衰 | β^λ | β^(λ-s) | s>0で減衰が少ない |
| 新規エラーへの減衰 | β^(λ-s-1) | 1（減衰なし） | 減衰が適用されない |

#### 具体例（λ=100, β=0.9995, s=50）

```
標準GNG:
  既存エラー: E × 0.9995^100 = E × 0.9512
  新規エラー: dist_sq × 0.9995^49 = dist_sq × 0.9758

論文Lazy:
  既存エラー: E × 0.9995^50 = E × 0.9753  (1.025倍多い)
  新規エラー: dist_sq × 1.0             (1.025倍多い)
```

#### 影響

1. **エラー蓄積の増加**: Lazy Errorは標準GNGより多くのエラーを蓄積
2. **ノード選択の違い**: 最大エラーノードの選択順序が変わる
3. **トポロジーの違い**: エッジ数や構造が異なる結果になる

#### ベンチマーク実測値

```
max_nodes=500:
  Standard GNG: 1264エッジ
  UG only:      1264エッジ ✓（完全一致）
  Lazy Error:   813エッジ  ✗（異なる）

max_nodes=1000:
  Standard GNG: 2631エッジ
  UG only:      2604エッジ ✓（ほぼ一致）
  Lazy Error:   異なる
```

#### 推奨事項

- **高速化のみ必要な場合**: `use_uniform_grid=True, use_lazy_error=False`（UGのみ）
  - 標準GNGと同等の結果で4〜8倍の高速化
- **論文完全再現が必要な場合**: `use_uniform_grid=True, use_lazy_error=True`
  - 論文の最大高速化を実現（ただし結果は標準GNGと異なる）
